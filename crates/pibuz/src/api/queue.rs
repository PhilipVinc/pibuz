// crates/pibuz/src/api/queue.rs — routes 13-16 (02-cli-and-api.md §3.3.13-16):
        ...
        match state.rt.block_on(state.runtime.core().get_track(*id)) {
            ...
            Err(_) => {
                return err_json(
                    ...
                    "check the track id: pibuz search <QUERY>",
                )
            }
        }
    }
    ...
    match check_remove_index(index, queue.total_tracks, queue.current_index) {
        ...
        RemoveCheck::PlayingIndex => {
            return err_json(
                ...
                "use: pibuz next, or pibuz queue clear",
            )
        }
        RemoveCheck::Ok => {}
    }
...
/// with no equivalent in a bare `pibuz queue add <TRACK_ID>` call (no album/
...
mod tests {
    ...
    fn remove_playing_index_error_body_matches_the_documented_hint() {
        ...
        // "message":"index 1 is the playing track","hint":"use: pibuz next, or pibuz queue clear"}}
