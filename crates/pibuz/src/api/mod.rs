// crates/pibuz/src/api/mod.rs — the HTTP control plane (02-cli-and-api.md §3).
//
...
/// Row 19 (this change): `GET /api/search` — caller: `pibuz search`. The
...
/// (foreign pibuz vs another process); everything else is a generic fatal.
#[derive(Debug)]
pub enum BindError {
...
/// check the response identifies as pibuz (`"app":"pibuz"`). Loopback, short
/// timeout, no dependency on reqwest (the CLI's async client is not built here).
pub fn probe_is_pibuz(addr: SocketAddr) -> bool {
...
pub fn serve(server: BoundServer, state: ApiState) -> ApiHandle {
    ...
    let thread = std::thread::Builder::new()
        ...
        .expect("failed to spawn pibuz api thread");
    ApiHandle {
        server: srv_handle,
        thread: Some(thread),
    }
...
fn route(state: &ApiState, req: &mut Request) -> Response<Cursor<Vec<u8>>> {
    ...
    match (method.as_str(), path.as_str()) {
        ("GET", "/api/ping") => json(
            200,
            serde_json::json!({"ok": true, "app": "pibuz", "api_version": crate::API_VERSION}),
        ),
        ...
        _ => err_json(404, "not_found", "unknown route", "see pibuz --help"),
    }
}
    ...
    fn response(&self) -> Response<Cursor<Vec<u8>>> {
        match self {
            ...
            GateReject::InvalidToken => err_json(
                ...
                "set QBZD_TOKEN or check [server] token in pibuz.toml",
            ),
        }
    }
}
...
mod tests {
    ...
    fn p1_route_table_grows_only_with_a_shipped_caller() {
        ...
        // GET /api/search — caller: `pibuz search`. Count is pinned so a route
        // with no caller cannot creep in; P1 must never overlap P0.
        assert_eq!(P1_ROUTES.len(), 27);
        assert!(P1_ROUTES.contains(&("GET", "/api/events"))); // caller: `pibuz watch`
        assert!(P1_ROUTES.contains(&("GET", "/api/artwork/current"))); // caller: `pibuz art`
