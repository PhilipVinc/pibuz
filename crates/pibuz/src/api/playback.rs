// crates/pibuz/src/api/playback.rs — routes 4-12 (02-cli-and-api.md §3.3.4-12):
...
/// api_version 1; needed so `pibuz now`'s stopped-state render, "stopped ·
...
pub fn play(state: &ApiState) -> Response<Cursor<Vec<u8>>> {
    ...
    // `pibuz now` / SSE show the transition to playing.
...
/// `{"mute": "on"|"off"|"toggle"}` (also `pibuz mute`'s route — no dedicated
/// route, §2.2).
...
/// latches into `last_errors.stream` (visible via `pibuz status`) and the log.
...
fn cold_start(state: &ApiState) -> Result<(), Response<Cursor<Vec<u8>>>> {
    ...
    let Some(track) = queue.current_track else {
        ...
        return Err(err_json(
            ...
            "queue a track first: pibuz queue add <TRACK_ID>",
        ));
    };
...
fn runtime_error(message: &str) -> Response<Cursor<Vec<u8>>> {
    err_json(500, "internal", message, "check: pibuz status")
}
...
fn device_error(message: &str) -> Response<Cursor<Vec<u8>>> {
    err_json(503, "audio_unavailable", message, "check: pibuz status")
}
