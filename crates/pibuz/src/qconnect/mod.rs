...
//! from pibuz.toml — only the daemon-root `qconnect_settings.db`.
...
pub(crate) struct DaemonQconnectInner {
    ...
    /// Latched connect/loop error; surfaced by `pibuz status` QConnect block (T11).
    #[allow(dead_code)]
...
fn latch_lifecycle_into_shared(shared: &SharedState, state: QconnectLifecycleState) {
    ...
    if let Ok(mut s) = shared.lock() {
        ...
        // Surface the transition on the CoreEvent bus (SSE, `pibuz watch`,
...
impl DaemonQconnectService {
    ...
    /// whatever `pibuz qconnect name` / `settings set qconnect.device_name` most
...
impl QconnectControl {
    ...
    pub async fn disconnect(&self) -> Result<(), String> {
        // Operator intent (`pibuz qconnect disable`): drop any handed-over
...
impl QconnectHandle {
    /// Connect on demand (T11 `pibuz qconnect enable`).
    #[allow(dead_code)]
    pub async fn connect(&self) -> Result<(), String> {
        self.service.connect().await
    }

    /// Disconnect on demand (T11 `pibuz qconnect disable`).
    #[allow(dead_code)]
    pub async fn disconnect(&self) -> Result<(), String> {
