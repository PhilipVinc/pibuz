// crates/pibuz/src/tui/app.rs — the setup-TUI state machine.
//
...
/// with them. What remains is what `pibuz setup` is now for — configuring the
...
impl App {
    pub fn new(roots: ProfileRoots, handle: Handle) -> Self {
        ...
        // exists — `pibuz setup` is now an audio/device configurator, and the
    ...
    fn enter_screen(&mut self, screen: Screen) {
        ...
        self.active = match screen {
            ...
            Screen::Network => {
                let (cfg, warns) = PibuzConfig::load(&self.roots.config_file())
                    .unwrap_or_else(|_| (PibuzConfig::default(), Vec::new()));
    ...
    /// Header row: `QBZ Daemon Setup` (accent-bold, left) · `pibuz <version>`
    /// (dim, right). One row, always visible.
    fn draw_header(&self, f: &mut Frame, area: Rect) {
        let title = s::APP_TITLE;
        let version = format!("pibuz {}", env!("CARGO_PKG_VERSION"));
...
fn save_network(
    ...
    match network_screen::rewrite_toml(&existing, bind, port, token) {
        Ok(text) => match std::fs::write(&path, text) {
            Ok(()) => None,
            Err(e) => Some(format!("failed to write pibuz.toml: {e}")),
        },
        Err(e) => Some(format!("failed to rewrite pibuz.toml: {e}")),
    }
}
...
mod tests {
    ...
    fn bare_app(section: Screen, focus: Focus) -> App {
        ...
        let active = match section {
            ...
            Screen::Network => {
                Active::Network(NetworkState::new(&PibuzConfig::default(), Vec::new()))
            }
