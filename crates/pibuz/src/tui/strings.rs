// crates/pibuz/src/tui/strings.rs — EVERY user-facing string in the setup TUI.
//
...
pub const NON_TTY_ERROR: &str = "error: 'pibuz setup' needs an interactive terminal
  → import a settings bundle:   pibuz settings import <file.qbzb>
  → set one value:              pibuz settings set <key> <value>
  → list what is settable:      pibuz settings list";
...
pub const APP_TITLE: &str = "Pibuz Setup";
...
pub const APPLIES_ON_START: &str =
    "changes apply when the daemon starts — systemctl --user status pibuz";
...
pub const N_DROP_UNKNOWN: &str =
    "note: pibuz.toml has keys outside the daemon schema — they are kept, but\n  comments and formatting are not preserved on save:";
