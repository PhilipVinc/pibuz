// crates/pibuz/src/tui/clipboard.rs — SSH-first tiered clipboard for the wizard's
...
pub enum Tier {
    ...
    Xclip,
    /// Guaranteed fallback: write to `~/pibuz-wizard/<name>.conf`.
    File,
}
...
/// Standard base64 with padding (no external crate — keeps the slim `pibuz`
...
pub fn wizard_dir() -> PathBuf {
    ...
    home.join("pibuz-wizard")
}

/// Write `text` to `~/pibuz-wizard/<stem>.conf`, creating the dir. Returns the
