// crates/pibuz/src/tui/screens/network.rs — the Network screen (03 §3.5).
//
// Edits ONLY [server] bind/port/token in pibuz.toml. The save is a whole-file
...
pub struct NetworkState {
    ...
    /// Unrecognized pibuz.toml keys (named in the pre-save warning, §3.5).
    unknown_keys: Vec<String>,
}

impl NetworkState {
    pub fn new(cfg: &PibuzConfig, unknown_keys: Vec<String>) -> Self {
        let staged = Staged {
...
pub fn rewrite_toml(
    ...
    let mut root: toml::Table = if existing.trim().is_empty() {
        ...
        toml::from_str(existing).map_err(|e| format!("cannot parse pibuz.toml: {e}"))?
    };
    let server = root
    ...
    let st = server
        .as_table_mut()
        .ok_or_else(|| "pibuz.toml [server] is not a table".to_string())?;
...
mod tests {
    ...
    fn rewrite_preserves_unknown_and_known_keys() {
        ...
        assert_eq!(parsed["data_root"].as_str(), Some("/srv/pibuz"));
