// crates/pibuz/src/tui/screens/wizard.rs — the HiFi/DAC Wizard (FB4), the setup
...
// block per DAC, `c`/`C` copy, `w` saves under ~/pibuz-wizard/ (never a system
...
impl WizardState {
    ...
    fn draw_test(&self, f: &mut Frame, area: Rect) {
        ...
        if self.tested {
            // Requested (what Pibuz asked the daemon for) vs negotiated (the DAC's
            // real hardware clock) — the bit-perfect proof.
            let req = match self.test_requested {
                Some((rate, bits)) if rate > 0 => {
                    format!("Pibuz requesting {} · {}-bit", wizard_core::khz(rate), bits)
                }
