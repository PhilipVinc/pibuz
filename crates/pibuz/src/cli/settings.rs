// crates/pibuz/src/cli/settings.rs — `pibuz settings show|set`,
// `pibuz qconnect enable|disable|name`, `pibuz config path|show`
            ...
            let mode = match raw.to_ascii_lowercase().as_str() {
                ...
                other => {
                    return Err(SetError::Usage(format!(
                        "invalid startup mode '{other}' — expected one of: on, off (use: pibuz qconnect enable|disable)"
                    )))
                }
            };
...
fn local_token(roots: &ProfileRoots) -> Option<String> {
    ...
    crate::config::PibuzConfig::load(&roots.config_file())
        .ok()
...
/// `pibuz settings show [--json]` (⬇). `--json`: `{"audio.backend": "alsa", ...}`
...
/// `pibuz settings set <KEY> <VALUE>` (⬇). Unknown key → exit 2 listing valid
...
/// `pibuz qconnect enable` (⬇). Writes `startup_mode = on`, nudges reload.
...
/// `pibuz qconnect disable` (⬇). Writes `startup_mode = off`, nudges reload
...
/// `pibuz qconnect name "Living Room"` (⬇). Empty clears the override back to
...
/// `pibuz config path` (⬇). Process roots + the credential file + the
...
fn unit_path() -> PathBuf {
    dirs::config_dir()
        ...
        .join("systemd/user/pibuz.service")
}

/// `pibuz config show [--json]` (⬇) — effective `pibuz.toml`, process concerns
...
pub fn config_show(json: bool, roots: &ProfileRoots) -> i32 {
    let path = roots.config_file();
    let (cfg, _warns) = match crate::config::PibuzConfig::load(&path) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("error: {e}");
            return 1;
...
/// Which dotted config keys the on-disk `pibuz.toml` actually sets (vs.
...
/// `pibuz settings export [FILE] [--from daemon|desktop] [--include-auth]` (⬇,
...
/// `pibuz settings import FILE [--include-auth] [--remap OLD=NEW]...
...
fn reload_disposition(
    ...
    match outcome {
        ...
        ReloadRefused => (
            ...
            Some(
                "error: settings saved but the daemon did not reload — restart it: systemctl --user restart pibuz"
                    .to_string(),
            ),
            1,
        ),
...
fn print_buckets(
    ...
    if bundle.source.profile == "desktop" {
            ...
            .find(|l| l.key == "qconnect.device_name")
        {
            ...
            println!(
                "  one name are hard to tell apart in the Qobuz app; rename with: pibuz qconnect name <name>"
            );
        }
    }
...
mod tests {
    ...
    fn present_keys_empty_for_a_missing_file() {
        ...
        let keys = present_keys(&roots.config.join("pibuz.toml"));
