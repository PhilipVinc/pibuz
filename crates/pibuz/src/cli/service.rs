// crates/pibuz/src/cli/service.rs — `pibuz service [systemd|openrc|runit]`, a
...
/// `pibuz service [INIT] [--user U] [--bin PATH] [--system]`. Prints the unit to
...
pub fn service(
    ...
    let init = match init.map(|s| s.to_ascii_lowercase()).or_else(detect_init) {
        ...
        None => {
            ...
            eprintln!("  → pibuz service systemd | openrc | runit");
            return 2;
        }
    };
...
fn resolve(user: Option<String>, bin: Option<String>) -> Target {
    let bin = bin
        ...
        .filter(|p| !p.is_empty())
        .unwrap_or_else(|| "/usr/bin/pibuz".to_string());
...
fn systemd_user(t: &Target) -> String {
    format!(
        "# pibuz.service — Pibuz headless Qobuz Connect daemon (systemd USER unit).\n\
         #\n\
         ...
         #   device vanishes from the Qobuz app. `pibuz status` warns when off.\n\
         # A user unit inherits your session env, so PipeWire/ALSA just work.\n\
         [Unit]\n\
         Description=Pibuz headless Qobuz Connect daemon\n\
         \n\
         [Service]\n\
         Type=simple\n\
         ExecStart={bin} run\n\
         ...
         # request returns EPERM and pibuz plays on at normal priority — see\n\
...
fn systemd_system(t: &Target) -> String {
    format!(
        "# pibuz.service — Pibuz headless Qobuz Connect daemon (systemd SYSTEM unit).\n\
         #\n\
...
fn openrc(t: &Target) -> String {
    format!(
        "#!/sbin/openrc-run\n\
         # pibuz — Pibuz headless Qobuz Connect daemon (OpenRC).\n\
         #\n\
...
fn runit(t: &Target) -> String {
    format!(
        "#!/bin/sh\n\
         # /etc/sv/pibuz/run — Pibuz headless Qobuz Connect daemon (runit).\n\
         #\n\
...
fn systemd_user_hint() -> String {
    "\n# Install (user unit):\n\
     #   pibuz service systemd > ~/.config/systemd/user/pibuz.service\n\
...
fn systemd_system_hint() -> String {
    "\n# Install (system unit):\n\
     #   pibuz service systemd --system | sudo tee /etc/systemd/system/pibuz.service > /dev/null\n\
     #   sudo systemctl daemon-reload\n\
     #   sudo systemctl enable --now pibuz\n"
        .to_string()
}

fn openrc_hint() -> String {
    "\n# Install:\n\
     #   pibuz service openrc | sudo tee /etc/init.d/pibuz > /dev/null\n\
     #   sudo chmod +x /etc/init.d/pibuz\n\
     #   sudo rc-update add pibuz default\n\
     #   sudo rc-service pibuz start\n"
        .to_string()
}

fn runit_hint() -> String {
    "\n# Install:\n\
     #   sudo mkdir -p /etc/sv/pibuz\n\
     #   pibuz service runit | sudo tee /etc/sv/pibuz/run > /dev/null\n\
     #   sudo chmod +x /etc/sv/pibuz/run\n\
     #   sudo ln -s /etc/sv/pibuz /var/service/    # Void; Artix: /run/runit/service\n"
        .to_string()
}
...
mod tests {
    ...
    fn systemd_user_has_execstart_and_no_user_env() {
        ...
        assert!(u.contains("ExecStart=/usr/local/bin/pibuz run"));
    ...
    fn system_templates_carry_the_audio_env_for_the_target_user() {
        for tpl in [systemd_system(&t()), openrc(&t()), runit(&t())] {
            ...
            assert!(tpl_has(&tpl, "/usr/local/bin/pibuz"), "missing bin:\n{tpl}");
    ...
    fn runit_execs_via_chpst_under_the_user() {
        ...
        assert!(r.contains("exec chpst -u alice:alice /usr/local/bin/pibuz run"));
    }
}
