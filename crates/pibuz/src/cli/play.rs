// crates/pibuz/src/cli/play.rs — the `pibuz play [CONTENT]` verb (02 §2.3).
//
// Bare `pibuz play` resumes / cold-starts the current queue (the shipped P0
...
pub async fn play(host: Option<String>, content: Option<String>, roots: &ProfileRoots) -> i32 {
    ...
    let body = match to_body(&content) {
        ...
        Err(msg) => {
            eprintln!("error: {msg}");
            eprintln!(
                "  → try: pibuz play album:<ID> | track:<ID> | artist:<ID> | playlist:<ID> | <qobuz-url>"
            );
            return 2;
        }
    };
