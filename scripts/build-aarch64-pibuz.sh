#!/usr/bin/env bash
#
# build-aarch64-pibuz.sh — build the Linux aarch64 (ARM64) `pibuz` daemon binary,
# e.g. for a Raspberry Pi 4 + HiFiBerry/USB DAC streamer.
#
# ── Why this is cheap ────────────────────────────────────────────────────────
# `pibuz` is slint-free: no UI crate, no fonts, no GPU libs. (Upstream's desktop
# `qbz` binary generates ONE ~1.6M-line `qbz_ui` module needing ~30 GB for a
# single rustc; this fork does not carry it.) pibuz builds in minutes and fits on
# modest hardware, so:
...
#                runs natively — see packaging/docker/pibuz-aarch64.Dockerfile)
#
# Output: dist/pibuz-aarch64-linux (an aarch64 ELF — verify with `file`).
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
  ...
  case "$os" in
    ...
    Linux)
      case "$arch" in
        ...
        *) echo "[pibuz-aarch64] ERROR: unsupported build host arch: $arch" >&2; exit 1 ;;
      esac
      ;;
    *) echo "[pibuz-aarch64] ERROR: unsupported build host OS: $os" >&2; exit 1 ;;
  esac
fi

case "$mode" in
  native)
    echo "[pibuz-aarch64] NATIVE build on $arch"
    if command -v apt-get >/dev/null; then
      sudo apt-get update
      sudo apt-get install -y --no-install-recommends "${DEPS[@]}"
    else
      echo "[pibuz-aarch64] non-apt distro: install the equivalents of: ${DEPS[*]}" >&2
    fi
    cargo build --release -p pibuz
    install -Dm755 "target/release/pibuz" "$OUT"
    ;;
  cross)
    echo "[pibuz-aarch64] CROSS-compile from $arch via cross (Docker)"
    if ! docker info >/dev/null 2>&1; then
      echo "[pibuz-aarch64] ERROR: Docker is not running. cross needs it." >&2
      exit 1
    fi
    command -v cross >/dev/null || cargo install cross --locked
    # Modest caps: the heaviest pibuz rustc is a fraction of the desktop's, so
    ...
    cross build --release --target "$TARGET" -p pibuz
    install -Dm755 "target/$TARGET/release/pibuz" "$OUT"
    ;;
  container)
    ...
    if ! docker info >/dev/null 2>&1; then
      echo "[pibuz-aarch64] ERROR: no container runtime. Start one, e.g.:" >&2
    ...
    docker run --rm --platform linux/arm64 \
      ...
      bash -euo pipefail -c '
        ...
        hits=$(cargo tree -p pibuz -e normal | grep -E "\bslint[a-z-]* v" || true)
        [ -z "$hits" ] || { echo "ERROR: pibuz graph resolves Slint:"; echo "$hits"; exit 1; }

        floor=$(objdump -T /target/release/pibuz \
                | grep -oE "GLIBC_[0-9]+\.[0-9]+" | sort -Vu | tail -1)
        echo "[pibuz-aarch64] glibc floor: $floor"
        highest=$(printf "%s\n" "$floor" "GLIBC_2.35" | sort -V | tail -1)
        [ "$highest" = "GLIBC_2.35" ] || { echo "ERROR: floor $floor exceeds 2.35"; exit 1; }

        install -Dm755 /target/release/pibuz /out/pibuz-aarch64-linux
      '
    ;;
  *)
